...
tp <- load_all_tables(folder_status, "tp") %>% replace(is.na(.), 0)
...
splitting <- load_all_tables(folder_status, "split") %>% replace(is.na(.), 0)
...
before_optim <- before_optim %>% 
    mutate(Max.Receiver = c(0, names(.))[f0(.) + 1])

before_optim$Max.Receiver <- sub("[^0-9]+", "", before_optim$Max.Receiver)
