...
#folder_status <- "../results/31-08-23_10-07-03"
#folder_status <- "../results/13-10-22_23-01-05/"
#folder_path <- "Methods"
...
coverage_data <- rename_benchs(coverage_data, benchs_small_names) %>%
  dplyr::mutate(Freq.Statement.Covered = round((Stmt.Cov / Num.Stmt)*100, 0), Num.Fn.Covered = round((Fn.Cov / Num.Fn)*100, 0))
...
dt_write_on_disk(table_caller_polymorphism, "table_caller_polymorphism", file.path(folder_status), bench_name = "all_benchs")
...
sites_per_targets <- before %>%
  select(Benchmark, contains("Num.Call.Sites"))
names(sites_per_targets) <- sub('(\\d+)_([a-zA-Z]+).(\\D+)', '\\1', names(sites_per_targets))
sites_per_targets <- sites_per_targets %>% tidyr::gather(key = Num.Targets, value = Num.Sites, -Benchmark)

calls_per_targets <- before %>%
  select(Benchmark, contains("Num.Calls"))
names(calls_per_targets) <- sub('(\\d+)_([a-zA-Z]+).(\\D+)', '\\1', names(calls_per_targets))
calls_per_targets <- calls_per_targets %>% 
  tidyr::gather(key = Num.Targets, value = Num.Calls, -Benchmark) %>%
  dplyr::mutate("Num.Targets" = as.numeric(Num.Targets), "Num.Calls" = as.numeric(Num.Calls))
#plots as facet of bar plots

#calls_per_targets <- calls_per_targets %>% filter(Benchmark == "DeltaBlue")
p <- ggplot(data=calls_per_targets, aes(x=Num.Targets, y=Num.Calls)) +
  geom_bar(stat="identity", fill="steelblue") +
  scale_y_log10() +
  theme_minimal() #+ theme(axis.text.x = element_text(angle=90, hjust=1))

p <- p + ggforce::facet_grid_paginate("Benchmark", ncol = 3, nrow = 4, page = 1, scales = "free_y")
...
list_mega <- table_one %>% select(Benchmark, Cluster.Type) %>% filter(Cluster.Type == "Polymorphic.Small")
list_mega <- list_mega$Benchmark
...
dep <- sum(table_two_aftertp %>% filter(Benchmark != "CD" & Benchmark != "RedBlack") %>% select(Poly.Calls))
arr <- sum(table_two_aftertp %>% filter(Benchmark != "CD" & Benchmark != "RedBlack") %>% select(TP_Poly.Calls))
((arr - dep)/dep) * 100
...
mini_table <- table_one %>%
  ...
  mutate_all(~replace(., is.nan(.), 0)) %>%
  add_percent_rounded(c("Change.Poly.Sites", "Change.Mega.Sites", "Change.Poly.Calls", "Change.Mega.Calls")) 
...
table_one <- table_one %>% 
  ...
  add_percent(c("Freq.Poly.Calls", "Freq.Poly.Call.Sites")) %>%
  add_percent_rounded(c("Stmt.Cov", "Fn.Cov")) %>%
  dplyr::arrange(Cluster.Type, Benchmark)
...
table_three_aftersplit <- table_three_aftersplit %>%
  ...
  add_percent(c("Change.Poly.Sites")) %>%
  add_percent_rounded(c("Change.Mega.Sites", "Change.Poly.Calls", "Change.Mega.Calls")) %>%
  dplyr::arrange(Cluster.Type, Benchmark)
...
table_split_transitions <- table_split_transitions %>%
  ...
  combine_similar_benchmarks(c("PsdUtil","PsdCompose","PsdImage","ChunkyCanvas","ChunkyColor")) %>%
  dplyr::arrange(Cluster.Type, Benchmark) %>% 
  add_percent_rounded(c("MONO->MONO (!=)", "MONO->MONO (=)"))
...
block_before <- sum_poly(block_before)

block_mono_sites <- sum(block_before %>% select(Mono.Call.Sites))
block_mono_total <- sum(block_before %>% select(Total.Call.Sites))
...
sum_blocks <- sum(block_before %>% select(Total.Calls))
