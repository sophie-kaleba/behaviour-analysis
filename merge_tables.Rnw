...
folder_out <- "./results/07-06-22_16-08-09/results/Status/"
...
load_all_tables <- function (folder, patt) { 
  result <- NULL
  files <- sort(list.files(folder, pattern=patt))
  
  for (f in files) {
      data <- read.csv(file.path(folder, f), sep=" ", strip.white = TRUE, header=TRUE) %>% select("Benchmark", everything())
...
before_optim <- load_all_tables(folder_out, "before") %>% replace(is.na(.), 0) %>% janitor::adorn_totals()
before_optim <- before_optim[rowSums(before_optim[,-3:-1]) > 0,] # filter out all the monomorphic-only benchmarks

tp <- load_all_tables(folder_out, "tp") %>% replace(is.na(.), 0) %>% janitor::adorn_totals()
tp <- tp[rowSums(tp[,-3:-1]) > 0,] # filter out all the monomorphic-only benchmarks

splitting <- load_all_tables(folder_out, "split") %>% replace(is.na(.), 0) %>% janitor::adorn_totals()
