...
data_split_transitions <- data_split_transitions %>% dplyr::arrange(Transition)  %>% select(-contains("..."))
...
table_data_tp_calls_test <- table_data_tp_calls %>%
    filter(str_detect(Benchmark, "PsdCompose")) %>%
    dplyr::mutate(across(!Benchmark, as.numeric)) %>%
    dplyr::summarise(across(!Benchmark, mean)) %>%
    dplyr::mutate_if(is.numeric, round, 1) %>%
    as.data.frame()
row.names(table_data_tp_calls_test) <- "Mean"

table_data_tp_calls_test_sd <- table_data_tp_calls %>%
    filter(str_detect(Benchmark, "PsdCompose")) %>%
    dplyr::mutate(across(!Benchmark, as.numeric)) %>%
    dplyr::summarise(across(!Benchmark, sd)) %>%
    dplyr::mutate_if(is.numeric, round, 1) %>%
    as.data.frame()
row.names(table_data_tp_calls_test_sd) <- "SD"

toto <- rbind(table_data_tp_calls_test, table_data_tp_calls_test_sd)
toto[1,]<-paste(toto[1,]," (",toto[2,],")", sep="")
toto[2,] <- NULL
