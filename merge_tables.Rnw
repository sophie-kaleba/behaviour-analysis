...
build_nice_latex_table <- function(df, caption_text = "", label_text = NULL) {
...
beauty <- kableExtra::kable(df, booktabs = TRUE, linesep = "", caption = caption_text, label = label_text) %>%
...
build_nice_latex_table_no_mega <- function(df, caption_text = "", label_text = NULL) {
# trash the data BUT looks exactly how it should for the paper
# see https://stackoverflow.com/questions/13984470/possible-to-create-latex-multicolumns-in-xtable/28420723#28420723
names(df)[2] = names(df)[4] = "Before"
names(df)[3] = names(df)[5] = "Variation (in \\%)"
# names(df)[4] = names(df)[7] = "Variation"
beauty <- kableExtra::kable(df, booktabs = TRUE, linesep = "", caption = caption_text, label = label_text, align = "r") %>%
  #kableExtra::row_spec(nrow(df), bold = TRUE, background = "#AFAFAF") %>%
  kableExtra::add_header_above(c("", "MONO"=2, "POLY"=2)) %>%
  kableExtra::kable_styling(latex_options = c("hold_position","scale_down", "striped"))
return(beauty)
}
...
# and all latexed
latex_tp_calls <- build_nice_latex_table_no_mega(table_data_tp_calls %>% select(-contains("MEGA")), "Addressing target polymorphism successfully reduce the amount of polymorphic calls by 62.8\\%", "tp_calls")
latex_tp_call_sites <- build_nice_latex_table_no_mega(table_data_tp_call_sites %>% select(-contains("MEGA")), "Addressing target polymorphism: impact on the number of polymorphic call-sites", "tp_call_sites")

latex_split_calls <- build_nice_latex_table_no_mega(table_data_split_calls %>% select(-contains("MEGA")), "Splitting deals with the remaining polymorphic calls, and monomorphize them all", "split_calls")
latex_split_call_sites <- build_nice_latex_table_no_mega(table_data_split_call_sites %>% select(-contains("MEGA")), "Applying splitting: impact on the number of polymorphic call-sites", "split_call_sites")

latex_data_split_transitions <- kableExtra::kable(data_split_transitions, booktabs = TRUE, linesep = "", caption = "A majority of splitting result in the call-sites having the same behavior before and after splitting, which is not ideal", label = "split_transitions", align = "r") %>%
...
\def\CallSites{%
<<CallSites, echo=FALSE, results='asis', fig.cap='Call-site optimizations impact on call-site behavior: data about call-sites', fig.subcap=c('Addressing target polymorphism: impact on the number of polymorphic call-sites', 'Applying splitting: impact on the number of polymorphic call-sites'), fig.ncol = 1>>=
print(latex_tp_call_sites)
print(latex_split_call_sites)
  @
  }%
