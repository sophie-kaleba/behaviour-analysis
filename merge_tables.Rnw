...
quantile(table_one$Max.Target)
quantile(table_two_aftertp$TP_Max.Target)
quantile(table_three_aftersplit$SPLIT_Max.Target)
############################################## STARTUP ###############################################
# Behaviour data - General
...
table_split_transitions %>% select(Benchmark, Times.Splitted) %>% arrange(Times.Splitted)
...
block_before <- block_before[ , gtools::mixedsort(names(block_before))] %>% select(c("Benchmark", "Num.LOC", "LOC.Cov", "Num.Fn", "Fn.Cov", "Times.Splitted"), everything())
...
block_splitting <- block_splitting[ , gtools::mixedsort(names(block_splitting))] %>% select(c("Benchmark", "Num.LOC", "LOC.Cov", "Num.Fn", "Fn.Cov", "Times.Splitted"), everything())
...
\newcommand{\BlockLowestPolySecond}{$\Sexpr{block_lowest_poly_freq$Freq.Poly.Call.Sites[[2]]}$}
