...
load_data_file <- function(filename, row_names) {
...
load_all_tables <- function (folder, head = TRUE, patt="") { 
  ...
  for (f in files) {
      ...
      data <- read.csv(file.path(folder, f), sep=" ", strip.white = TRUE, header=head, check.names = FALSE)
...
#before_optim <- before_optim[rowSums(before_optim[,-3:-1]) > 0,] # filter out all the monomorphic-only benchmarks
...
#tp <- tp[rowSums(tp[,-3:-1]) > 0,] # filter out all the monomorphic-only benchmarks
...
#splitting <- splitting[rowSums(splitting[,-3:-1]) > 0,] # filter out all the monomorphic-only benchmarks
...
sum_poly <- function(df) {
  df <- df %>% mutate(Max.Receiver = c(0, names(.))[f0(.) + 1])
  df$Max.Receiver <- sub("[^0-9]+", "", df$Max.Receiver)
...
table_one <- before_optim %>%
  select(Benchmark, LOC, LOC.Cov, Num.Fn, Fn.Cov, Max.Receiver, Poly.Call.Sites, Poly.Calls, Mega.Call.Sites, Mega.Calls) %>%
...
# For table 2, we need to have 2 temporary tables, one before optim, one after tp
table_before_aux <- before_optim %>%
  select(Benchmark, `1_Num.Call.Sites`, `1_Num.Calls`, Poly.Calls, Poly.Call.Sites, Mega.Calls, Mega.Call.Sites, Max.Receiver) %>%
  mutate(Poly.Call.Sites = Poly.Call.Sites + Mega.Call.Sites, Poly.Calls = Mega.Calls + Poly.Calls, across(!Benchmark, as.numeric)) %>%
  select (-Mega.Call.Sites, -Mega.Calls)
...
# from this table, we can extract Table3, for both calls and call-sites. Beware, we still need the amount of time they have been split
table_three_global <- table_tp_aux %>%
  merge(table_split_aux) %>%
  filter(TP_Poly.Call.Sites > 0) %>%
...
# This tabke displays to which extent the optimizations impacted polymoprhism, this is new from the article
table_four_extent <- table_before_aux %>%
  merge(table_tp_aux) %>%
  merge(table_split_aux) %>%
  select(Benchmark, Max.Receiver, TP_Max.Receiver, SPLIT_Max.Receiver) %>%
  filter(Max.Receiver > 1)
############################################## UTILS ###############################################  
