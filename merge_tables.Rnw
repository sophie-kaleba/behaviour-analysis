...
combine_similar_benchmarks <- function(df, benchmark_prefix) {
  ...
  table_mean[1,]<-paste("[avg] ", table_mean[1,], sep="")
...
# trash the data BUT looks exactly how it should for the paper
# see https://stackoverflow.com/questions/13984470/possible-to-create-latex-multicolumns-in-xtable/28420723#28420723
names(df)[2] = names(df)[5] = "Before"
names(df)[3] = names(df)[6] = "After"
names(df)[4] = names(df)[7] = "Variation"
beauty <- kableExtra::kable(df, booktabs = TRUE, linesep = "", caption = caption_text, label = label_text) %>%
  kableExtra::row_spec(nrow(df), bold = TRUE, background = "#AFAFAF") %>%
  kableExtra::add_header_above(c("", "MONO"=3, "POLY"=3)) %>%
  kableExtra::kable_styling(latex_options = c("hold_position","scale_down", "striped"))
return(beauty)
}
...
latex_data_split_transitions <- kableExtra::kable(data_split_transitions, booktabs = TRUE, linesep = "", caption = "A majority of splitting result in the call-sites having the same behavior before and after splitting, which is not ideal", label = "split_transitions") %>%
