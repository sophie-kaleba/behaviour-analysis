...
Num.Call.Sites <- n_distinct(total$Source.Section, total$Symbol) 
Num.Call.Sites.Incl.Splitted <- n_distinct(total$Source.Section, total$Symbol, total$CT.Address) 
Num.Targets.Original <- n_distinct(total$Source.Section, total$Symbol, total$Num.Receiver.Original) 
Num.Targets.Observed <- n_distinct(total$Source.Section, total$Symbol, total$Num.Receiver.Observed) 
...
compute_num_target <- function(num_receiver_column) {
  df <- total %>%
...
num_target_original <- compute_num_target("Num.Receiver.Original")
num_target_observed <- compute_num_target("Num.Receiver.Observed")
...
#' @param num_receiver_column, whether we consider the observed or the original number of receivers  
#' @param ct_address, whether we consider splitting 
compute_num_target_splitting <- function(num_receiver_column) {
  df <- total %>%
...
num_target_observed_inc_splitting <- compute_num_target_splitting("Num.Receiver.Observed")
assert("... And it should match the total number of call-sites", first(num_target_observed_inc_splitting$Cumulative) == Num.Call.Sites.Incl.Splitted)
