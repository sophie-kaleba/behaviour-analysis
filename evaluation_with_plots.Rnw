...
# split_truffle <- read.csv("./truffle_split_targets_toto", sep=",", row.names = NULL, col.names=c("Target","Times.Splitted", "Call.Type"))
...
mono <- transition_data %>% filter(Start.State == 'MONOMORPHIC' & End.State == 'MONOMORPHIC')
...
lfgjdlfjg <- total_w_splitting %>%
            filter (Symbol == "capitalize" & CT.Address == 1504 & Source.Section == "/home/sopi/Documents/Side_projects/truffleruby/bench/logging/DocSplitting.rb:4:1:21")
...
now <- paste("results",folder_out,paste(benchmark_name,now,sep="_"),sep="/")
dir.create(now)
...
split_lifetime <- function(df, ss, symb) {
  ...
  p <- ggplot(data = filtered_data, aes(x=as.numeric(as.character(Call.ID)), y=as.character(CT.Address), color=Cache.Type.Observed)) + geom_point(size = 0.5, alpha = 0.4) 
