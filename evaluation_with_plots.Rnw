...
compute_num_target <- function(num_receiver_column) {
  df <- total %>%
  select(Source.Section, Symbol,!! sym(num_receiver_column)) %>% # the sym trick is necessary so that dplyr processes the parameter
  ...
  dplyr::arrange(!! sym(num_receiver_column)) %>%
