...
data <- read_splitting_profiling_file("./parsed_DeltaBlue.mylog")
...
summary_cache_change_detail <- total_w_splitting %>%
      ...
      slice(1) %>%
      mutate(Cumulative = rev(cumsum(rev(Num.Call.Sites))))
      #verify(first(Cumulative) == Has.Changed.Status) # the cumulative amount of TP call-site should be equal to Has.Changed.Status TODO-CHECK ##################################################################
...
generate_splitting_summary <- function(data, ...) {
  df <- data %>%
    ...
    dplyr::mutate(Num.Calls = n()) %>%
    dplyr::mutate(Frequency = round(Num.Calls/nrow(data),3)*100)
  return(df)
}
