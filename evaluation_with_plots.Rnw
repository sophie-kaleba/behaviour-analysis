...
summary_splitting_detail_turbo <- generate_splitting_summary(total_w_splitting, col) %>%
  group_by(Source.Section, Symbol, CT.Address) %>%
  slice(1) %>%
  dplyr::arrange(desc(Times.splitted)) 
...
distrib_cache_size_obs_no_splitting <- autoformat(num_target_observed, zap = getOption("digits"))
...
distrib_cache_size_original_no_splitting <- autoformat(num_target_original, zap = getOption("digits"))
...
distrib_cache_size_obs_splitting <- autoformat(num_target_observed_w_splitting, zap = getOption("digits"))
...
distrib_cache_size_original_splitting <- autoformat(num_target_original_w_splitting, zap = getOption("digits"))

# Target polymorphism
target_polymorphism <- xtable(summary_cache_change, caption = "Distribution of call-targets (Observed) - Splitting ignored")
target_polymorphism <- autoformat(summary_cache_change, zap = getOption("digits"))
...
target_polymorphism_detail <- autoformat(summary_cache_change_detail, zap = getOption("digits"))
...
target_polymorphism_turbo <- autoformat(summary_cache_change_detail_turbo, zap = getOption("digits"))
