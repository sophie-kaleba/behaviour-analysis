...
############################################## SPLITTING ##############################################    
test_data <- read_test_data("./total_w_splitting.csv")
test_data <- test_data %>% dplyr::select(Call.ID, Symbol, Source.Section, CT.Address, Observed.Receiver, Benchmark, Num.Receiver.Observed, Cache.Type.Observed, Times.splitted, Num.Calls)

# per call-site
test <- test_data %>%
  group_by_at(Call.Site) %>%
  dplyr::mutate(Times.splitted = n_distinct(CT.Address) - 1, Num.Calls = n_distinct(Call.ID))
write.csv(test,"./test_splitting.csv", row.names = FALSE)

dfjgkdfgkd <- total_w_splitting %>%
    group_by_at(Call.Site) %>%
    dplyr::mutate(Times.splitted = n_distinct(CT.Address) - 1, Num.Calls = n_distinct(Call.ID))
dfjgkdfgkd <- dfjgkdfgkd %>% dplyr::select(Call.ID, Symbol, Source.Section, CT.Address, Observed.Receiver, Benchmark, Num.Receiver.Observed, Cache.Type.Observed, Times.splitted, Num.Calls)
write.csv(dfjgkdfgkd,"./real_data.csv", row.names = FALSE)
...
real_data <- read.csv("./splitting-analysis/test_output.csv", header = FALSE, sep = ",", row.names=NULL, col.names=row_names)
...
rd <- real_data %>%
  dplyr::mutate(Transition.Type = case_when( Start.State == 'POLYMORPHIC' & End.State == 'POLYMORPHIC' & Intersect.Size == Union.Size ~ "Same",
                                             ...
                                             TRUE ~ "Different")) %>%
  group_by(Start.State, End.State, Transition.Type) %>%
  dplyr::summarise(Num.Transitions = n())
  

# keep the receiver so it is easier to compare with Truffle's splitting summary
...
dfjgkdfgkd <- total_w_splitting %>%
    ...
    dplyr::arrange(Call.ID, .by_group = TRUE) %>%
    ...
    select(Call.ID, Source.Section, Symbol, CT.Address, Times.splitted, Cache.Type.Observed, Group.ID, Sub.Group.ID) %>%
...
effect_splitting_summary <- effect_splitting %>%
  ...
  dplyr::summarise(Num.Call.Sites = n())
