...
dt_write_on_disk(sum_bench, "table_distribution", file.path(folder_out,"General"))
...
for (p in distribution_plots_dynamic) {
  # todo implement a function to save the plots in the distribution folder ~ or the plots folder, that would need to be created here and removed from the makefile
}