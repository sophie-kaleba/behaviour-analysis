...
detailed_table_caller_polymorphism <- data %>% 
  ...
  select(Benchmark, Source.Section, Observed.Receiver, Symbol, Call.Site.ID, CT.Address, Sites.Per.Method, Split.Sites.Per.Method) %>%
  distinct(Benchmark, Source.Section, Observed.Receiver, Symbol, Call.Site.ID, CT.Address, .keep_all = TRUE)
