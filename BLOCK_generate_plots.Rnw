...
if (!keep_startup) {
  ...
  stop("Only keep startup data for plots for now")
}
...
data[ , Num.Receiver.Original := n_distinct(Original.Receiver), by=list(Source.Section, Symbol, Call.Site.ID, Benchmark)]
...
data_plot <- data[Lookup.Status != "MONO"]
...
plot_calls <- function(ss, symb, site) {
  ...
  p <- ggplot(data = filtered_data, aes(x=as.numeric(as.character(Call.ID)), y=Observed.Receiver, color=Observed.Receiver)) + geom_point(size = 2, alpha = 0.4) 
...
f <- function(row) {
  ...
  ggsave(p_, path=folder_out, filename=paste(rank,"BLOCK",ss,symbol,"plot.png", sep="_"), height = 300, width = 500, units="mm")
}
...
f <- function(row) {
  ...
  ss <- gsub("*/","_",ss) # replace the / in the name otherwise the file cannot be created
  ss <- str_trunc(ss, 25, side='left')
  ggsave(p_, path=split_lifetime_folder, filename=paste(rank,"BLOCK","splitlifetime",ss,symbol,"plot.png", sep="_"), height = 300, width = 500, units="mm")
}
