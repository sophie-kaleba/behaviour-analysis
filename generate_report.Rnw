...
folder_status <- "../results/latest/General/"
folder_cov <- "../results/latest/Coverage/Global/"
...
before <- before[ , gtools::mixedsort(names(before))] %>% select(c("Benchmark", "LOC", "LOC.Cov", "Num.Fn", "Fn.Cov", "Times.Splitted"), everything())

tp <- data %>%
  select(1:6, contains("TP"))
names(tp) <- sub('(\\d+)_([a-zA-Z]+).(\\D+)_', '\\1_\\3', names(tp))
tp <- tp[ , gtools::mixedsort(names(tp))] %>% select(c("Benchmark", "LOC", "LOC.Cov", "Num.Fn", "Fn.Cov", "Times.Splitted"), everything())

splitting <- data %>%
  select(1:6, contains("Split"))
names(splitting) <- sub('(\\d+)_([a-zA-Z]+).(\\D+)_', '\\1_\\3', names(splitting))
splitting <- splitting[ , gtools::mixedsort(names(splitting))] %>% select(c("Benchmark", "LOC", "LOC.Cov", "Num.Fn", "Fn.Cov", "Times.Splitted"), everything())
...
table_one <- before %>%
  select(Benchmark, LOC, LOC.Cov, Num.Fn, Fn.Cov, Max.Target, Mono.Call.Sites, Mono.Calls, Poly.Call.Sites, Poly.Calls, Mega.Call.Sites, Mega.Calls) #%>%
