...
table_after_tp_details <- load_all_tables(folder_details, patt="tp")
...
table_after_tp_details_latex <- kableExtra::kable(table_after_tp_details %>% select(-Benchmark), booktabs = TRUE, align = "r")
...
\def\PolyAfterTP{%
<<PolyAfterTP, echo=FALSE, results='asis'>>=
print(table_after_tp_details_latex)
  @
  }%
  
\def\PolyAfterSplitting{%
<<PolyAfterSplitting, echo=FALSE, results='asis'>>=
