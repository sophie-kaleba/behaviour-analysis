...
# folder_out = "/home/sopi/Documents/Side_projects/ruby-cs-analyser/results/latest"
# filename = "/home/sopi/Documents/Side_projects/ruby-cs-analyser/results/latest/parsed_DeltaBlue.log"
# keep_startup = FALSE
# keep_blocks = TRUE
...
if(!keep_blocks) {
  ...
  stop("The file should contain only blocks")
} else {
...
data[ , Num.Receiver.Original := n_distinct(Original.Receiver), by=list(Source.Section, Symbol, Benchmark)]
...
split_table <- split_table[ , .(Start.Time = first(Call.ID),
                    End.Time = last(Call.ID),
                    Observed.Receiver), by = list(Benchmark, Source.Section, Symbol, CT.Address) ]
...
table_two_before <- dt_generate_table_two(data, Observed.Receiver, Original.Receiver, c("Source.Section", "Symbol"), Before.Num.Targets) %>% filter(Cache.Type != "MONO")
...
if (!dir.exists(file.path(folder_out,"General")) & !dir.exists(file.path(folder_out,"Details"))) {
