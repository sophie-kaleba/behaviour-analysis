...
split_lifetime <- function(ss, symb) {
  ...
  p <- ggplot(data = filtered_data, aes(x=as.numeric(as.character(Call.ID)), y=as.character(CT.Address), color=as.character(Signature))) + geom_point(size = 0.5, alpha = 0.4) 
