...
data_plot[ , "N.Calls" := uniqueN(.SD) , by = list(Source.Section, Symbol, Observed.Receiver)]
...
data_split <- data[Times.Splitted > 0 & TP.Lookup.Status == "POLY"]
setorderv(data_split, c("Times.Splitted"), order=-1L)

data_split <- data_split[ , .(Observed.Receiver = list(Observed.Receiver), Call.ID), by = list(Source.Section, Symbol, CT.Address)]
data_split <- data_split[ , "Signature" := lapply(.SD[[1]], unique), .SDcols = c("Observed.Receiver")] 
data_split[, Observed.Receiver := NULL]
...
split_lifetime <- function(ss, symb) {
  ...
  p <- ggplot(data = filtered_data, aes(x=as.numeric(as.character(Call.ID)), y=as.character(CT.Address), color=Signature[[1]])) + geom_point(size = 0.5, alpha = 0.4) 
...
f <- function(row) {
  ...
  ss <- gsub("*/","_",ss) # replace the / in the name otherwise the file cannot be created
  ss <- str_trunc(ss, 25, side='left')
  ggsave(p_, path=folder_out, filename=paste(rank,"splitlifetime",ss,symbol,"plot.png", sep="_"), height = 300, width = 500, units="mm")
}
