...
transition_data[ , .("Num.Transitions" := n_distinct(Source.Section, Symbol)), by=list(Benchmark, Start.State, End.State, Transition.Type)]
...
table_one <-  table_one[data[ , Times.Splitted := 
                              by = "Split.Num.Targets"]]
