...
# to recreate the generate_table_one method
# first do the mutate_per_state
# WIP - likely the "Num.Receiver" part will change if I only keep one table instead of splitting into several
test <- before_all_optim[ , .(Num.Calls = n_distinct(Call.ID), 
...
test_before <- mutate_per_state(before_all_optim, "Num.Receiver")
